...
function [prod_max,Program1,Program2]=Optimise_Production(L,L2,o,T,N,type_retenue1,type_retenue2,vol_depart,vol_reserve_depart,P_H_Pleine,P_H_Creuse,Periode_H)
    % Données
    rho=1000;                      
    g=9.80665;
    mu1=0.2;
                        ...
                        if(Volume2(k)+W2(i)-Transfert(t)>=0)
                            for u=1:o+1
                                if((Volume(j)+W1(i)-Controle(u)+Transfert(t)>=0))
                                    optimal2(u,t)=rho*g*(Volume(j)*mu1*Controle(u)+Volume2(k)*mu2*Transfert(t))*Prix(i) + V(i+1,round(max(min(L,Volume(j)+W1(i)-Controle(u)+Transfert(t)),1)),round(max(min(L2,Volume2(k)+W2(i)-Transfert(t)),1)));
                                end
                            end
